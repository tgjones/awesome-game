	...
	class Camera
	{
		...
		public void Update(GameTime gameTime, GraphicsDevice graphicsDevice)
		{
			...
			position = new Vector3(
				...
				(float) Math.Sqrt((float) ((maxX - minX) * (maxX - minX)) + ((maxZ - minZ) * (maxZ - minZ) * (aspectRatio * aspectRatio))) * (float) Math.Tan(MathHelper.ToRadians(45f)) + 20 + maxY + 50,
				(float)sumZ / viewObjects.Count + 60);
